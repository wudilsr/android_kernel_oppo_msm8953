...
void ktd22xx_lowbattery_breath_leds(void) {
	...
	i2c_smbus_write_byte_data(ktd20xx_client, 0x06, 0xbf); /* set current to 24mA */
	i2c_smbus_write_byte_data(ktd20xx_client, 0x05, 0xaa); /* rise time */
	...
	i2c_smbus_write_byte_data(ktd20xx_client, 0x02, 0x56); /* led flashing (current ramp-up and down continuously) */
}
...
void ktd2xx_led_on(void) {
	...
	i2c_smbus_write_byte_data(ktd20xx_client, 0x06, 0x2f); /* set current to 10mA */
	i2c_smbus_write_byte_data(ktd20xx_client, 0x04, LED_OUT_ALON); /* turn on all of the LEDs */
}

void ktd2xx_led_off(void) {
	/* turn off LED when 0x02 is not 0x00, and is set to the same as breath LEDs */
	i2c_smbus_write_byte_data(ktd20xx_client, 0x06, 0x00); /* set current to 0.125mA */
...
void ktd22xx_breath_leds_time(int blink) {
	...
	pr_info("ktd20xx led write blink = %x, period = %x, flashtime = %x\n", blink, period, flashtime);
	i2c_smbus_write_byte_data(ktd20xx_client, 0x00, 0x20); /* initialization LED off */
	i2c_smbus_write_byte_data(ktd20xx_client, 0x04, 0x00); /* initialization LED off */
	i2c_smbus_write_byte_data(ktd20xx_client, 0x06, 0xbf); /* set current to 24mA */
	i2c_smbus_write_byte_data(ktd20xx_client, 0x05, period); /* rise time */
			 ...
			 const char *buf, size_t count)
{
	...
	if (~(~value) && totalMS && onMS) {
		...
		pr_info("%s: total_MS:%d, onMS:%d, totalMS_reg:%d, onMS_reg:%d, stableMS_reg:%x\n",
				__func__, totalMS, onMS, totalMS_reg, onMS_reg, stableMS_reg);
		pr_info("%s: ktd20xx color_D1:%d color_D2:%d color_D3:%d\n",
				__func__, color_D1, color_D2, color_D3);

		i2c_smbus_write_byte_data(ktd20xx_client, 0x00, 0x20); /* initialization LED off */
		i2c_smbus_write_byte_data(ktd20xx_client, 0x04, 0x00); /* initialization LED off */
		i2c_smbus_write_byte_data(ktd20xx_client, 0x06, color_D1); /* set current to D1mA */
		i2c_smbus_write_byte_data(ktd20xx_client, 0x07, color_D2); /* set current to D2mA */
		i2c_smbus_write_byte_data(ktd20xx_client, 0x08, color_D3); /* set current to D3mA */
		i2c_smbus_write_byte_data(ktd20xx_client, 0x05, onMS_reg); /* rise time */
		...
		i2c_smbus_write_byte_data(ktd20xx_client, 0x04, LED_OUT_PWM1); /* allocate led1 to timer1 */
		i2c_smbus_write_byte_data(ktd20xx_client, 0x02, stableMS_reg); /* LED flashing (current ramp-up and down continuously) */
	} else {
...
static int ktd20xx_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_info("oppo_led_debug %s is probe start \n", __func__);
	...
	for (i = 0; i < 1; i ++) {
		if (led_classdev_register(&client->dev, &KTD2026_lcds[i]))
			pr_err("led_classdev_register failed of KTD2026_lcds!\n");
	}
	...
	/* turn off LEDs when first start ktd */
	ret = i2c_smbus_write_byte_data(ktd20xx_client, 0x00, 0x05); /* reset reg */
	ret = i2c_smbus_write_byte_data(ktd20xx_client, 0x06, 0x00); /* set current to 0.125mA */
	...
	if (ret < 0) {
		pr_err("can't find ktd2026 led control ic!");
		goto err_group_register;
	}
	...
	pr_info("oppo_led_debug %s is probe success end \n", __func__);
	return 0;
...
static int __init ktd20xx_init(void)
{
	...
	if (err)
		pr_err("ktd20xx driver failed (errno = %d)\n", err);
	else
