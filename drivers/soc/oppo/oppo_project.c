		...
		size_t count,loff_t *off)
{
	...
	pr_info("lycan test secure_oem_config 0x%x\n", secure_oem_config);
...
#define QFPROM_RAW_SERIAL_NUM 0x000a0128

static unsigned int g_SerialID = 0x00;
...
static int __init btb_lbc_sts_init(void)
{
	...
	if (NULL == substr) {
		btb_lbc_sts = false;
		pr_info("%s: OPPO_BTB: btb_flag not find in cmdline\n", __func__);
		return 0;
	}
		...
		size_t count,loff_t *off)
{
	...
	pr_info("%s: OPPO_BTB: lbc_read_proc: %d\n", __func__, low_bat_chg);
